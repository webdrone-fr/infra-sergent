            ...
            case "update-modules":
                ...
                if (resultGitpull.contains("output")){
                    try {
                        ...
                        result = execute("docker", ("exec -t "+ stackName +"-meveo curl http://localhost:8080/meveo/api/rest/module/initDefault -u " + username + ":" + password + " -H 'Content-Type:application/json' --max-time "+ timeoutSec +" -d '" + params + "'").split("\\s+"));
